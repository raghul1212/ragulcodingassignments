...
public class ProductImplementationUsingStream {
	...
	for(Product p:productList) {
		...
		return sum;
		*/
	int sum=productList.stream().map(p->p.price).reduce(0, (a,b)->a+b);
	return sum;
	...
	int  maxPrice(ArrayList<Product> productList) {
	   /* int maxi=Integer.MIN_VALUE;
		...
		return maxi; */
		return productList.stream().map(p->p.price).max((p,q)->p.compareTo(q)).get();
	...
	int  minPrice(ArrayList<Product> productList) {
		/* int min1=Integer.MAX_VALUE;
			...
			return min1; */
		return productList.stream().map(p->p.price).min((p,q)->p.compareTo(q)).get();
		
		
	}
	List<String> getProductNameList(ArrayList<Product> productList) {
		//List<String> namesList=new ArrayList<String>();
		return productList.stream().map(obj->obj.getName()).collect(Collectors.toList());
	/*	for(Product obj:productList){
